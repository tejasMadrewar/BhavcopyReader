...
def clean_df(date, df):
    ...
    df = df.astype({"TRADES": "float", "HI_52_WK": "float32", "LO_52_WK": "float32"})
    # make DATE1 column
    ...
    df = df.astype({"mkt": "string", "series": "string", "symbol": "string"})
...
def zipfile_to_pd_df(date, zipfile_obj):
    ...
    # print(date, Pd_file)
    Pd_file = list(filter(lambda x: x.endswith(Pd_file), zipfile_obj.namelist()))[0]
...
def days_to_df(days, folder):
    ...
    df = df.astype({"mkt": "category", "series": "category", "symbol": "category"})
